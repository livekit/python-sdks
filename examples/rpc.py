...
async def register_receiver_methods(greeters_room: rtc.Room, math_genius_room: rtc.Room):
    async def arrival_method(request_id: str, caller: rtc.RemoteParticipant, payload: str, response_timeout_ms: int):
        print(f"[Greeter] Oh {caller.identity} arrived and said \"{payload}\"")
        await asyncio.sleep(2)
        return "Welcome and have a wonderful day!"

    async def square_root_method(request_id: str, caller: rtc.RemoteParticipant, payload: str, response_timeout_ms: int):
        json_data = json.loads(payload)
        number = json_data['number']
        print(f"[Math Genius] I guess {caller.identity} wants the square root of {number}. I've only got {response_timeout_ms / 1000} seconds to respond but I think I can pull it off.")
    ...
    await math_genius_room.local_participant.register_rpc_method('square-root', square_root_method)
...
async def perform_square_root(room: rtc.Room):
    ...
    try:
        response = await room.local_participant.perform_rpc('math-genius', 'square-root', json.dumps({"number": 16}))
